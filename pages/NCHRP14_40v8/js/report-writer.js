$(document).ready(function() {
	$('#calc-ms').click(function() {
		var totalScore = calculateMS();
		console.log(totalScore);
		// creates modal that informs you of your score
		var msMes = managedSuccession(totalScore);
		$('p', "#msModal")[0].innerHTML = "Managed Succession: " + msMes;
		var msmod = $('#msModal');
		msmod.prop("style").display = "block";
	});

	var modbtn = document.getElementById("msmodBtn");

	modbtn.onclick = function() {
		var msmod = $('#msModal');
		msmod.prop("style").display = "none";

	}

	window.onclick = function(event) {
		var msmod = document.getElementById('msModal');
		if (event.target == msmod) {
			msmod.style.display = "none";
		}
	} 
	...
	$('#download-report').on("click", function() {
		...
		// pdf.setFontSize(16);
		pdf.text(XMG, ycoord, "Roadside Management Selection Report\n");
...
// 				text += addtext;
				quesCell = [label.innerHTML, $("#" + labelFor, "#slide-two-content")[0].value];
				...
				if ($(labeln).attr("id") == ("a-class" || "b-class"|| "c-class")) {
					if (label == "yes") {
						label = "A";
					}
					else if (label == "no1") {
...
// 				console.log(addtext);
		...
		// sets the y coordinates of the managed succession text
		ycoord = 560;
		pdf.text(XMG, ycoord, "Managed Succession: " + managedSuccession(calculateMS()));
// 		console.log(text);
...
// 		ycoord += countLines(text, "</p>");
		...
		//saves pdf file
		pdf.save("NCHRP 14-40 Tool Report.pdf");
...
function calculateMS() {
	var totalScore = 0;
	const QUESTIONS = $("label", '#slide-two-content');
	for (var i = 0; i < QUESTIONS.length; i++) {
		var label = $('input:checked', $(QUESTIONS[i]).parents()[0]).val();
		var labeln = $('input:checked', $(QUESTIONS[i]).parents()[0]);
		// checks if item is selected
		if (label != undefined) {
			// grabs the point value from each selected element and converts to int
			var pointval = parseInt($(labeln).attr('points'), 10);
			totalScore += pointval;
		}
	}
	return totalScore;
...
function managedSuccession(totalScore) {
	if (totalScore > 30) {
